...
if (!function_exists('addVideo')) {
    function addVideo($title, $director, $release_year, $user_id, $video_data) {
        global $conn;
        $sql = "INSERT INTO videos (title, director, release_year, user_id, video_data) VALUES (?, ?, ?, ?, ?)";

        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("sssib", $title, $director, $release_year, $user_id, $video_data);
            $stmt->send_long_data(4, $video_data); // Bind the longblob data separately
            $stmt->execute();
            $stmt->close();
        }
    }
